...
import { WorkloadHeatmap } from '@/shared/mission-control/MissionCalendar'
import {
  buildAiSuggestions,
  heatmapDays,
  projectHoursForDay,
...
export function MissionOverview({
  ...
  onSelectDay,
  onApplyBalance,
  resolveTaskHref = (taskId) => `/personal/tasks/${taskId}`,
  resolveProjectHref = (projectId) => `/personal/projects/${projectId}`,
}: {
  dayKey: string
  tasks: TaskWithProject[]
  projects: MissionProjectInsight[]
  projectFilter: string | 'all'
  focus: TaskWithProject | null
  onSelectDay: (dayKey: string) => void
  onApplyBalance: () => void
  resolveTaskHref?: (taskId: string) => string
  resolveProjectHref?: (projectId: string) => string
}) {
      ...
      {focus ? (
        <div className="rounded-2xl border border-accent/30 bg-accent/5 p-4">
          ...
          <Button asChild size="sm" variant="secondary" className="mt-3">
            <Link to={resolveTaskHref(focus.id)}>{t('mission.openFocus')}</Link>
          </Button>
        </div>
      ) : null}
