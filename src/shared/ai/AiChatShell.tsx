...
function noopExpandCreateMany(_action: ParsedRegistryAction) {
...
export type AiChatShellProps = {
  mode?: 'personal' | 'workspace'
  workspaceId?: string
  workspaceRole?: WorkspaceRole | null
  previewDirectory: PreviewDirectory
  onInvalidateAfterActions: () => Promise<void>
  coalesceCreates?: boolean
  expandCreateManyForDisplay?: (action: ParsedRegistryAction) => Array<{ key: string; label: string }>
  transformProposedActions?: (actions: AiAction[]) => AiAction[]
}
...
export function AiChatShell({
  ...
  workspaceRole,
  previewDirectory,
  onInvalidateAfterActions,
  coalesceCreates = false,
  expandCreateManyForDisplay = noopExpandCreateMany,
  transformProposedActions,
}: AiChatShellProps) {
  ...
  function applyProposed(raw: AiAction[], conversationId?: string, userMessage?: string) {
    ...
    const next = transformProposedActions ? transformProposedActions(rewritten) : rewritten
  ...
  async function invalidateAfterActions() {
    await onInvalidateAfterActions()
  }
