...
export function NoteEditorPage() {
  ...
  const convert = useMutation({
    ...
    onSuccess: async (task) => { await qc.invalidateQueries({ queryKey: tasksKeys.all }); toast.success('Task created from note'); window.location.assign(`/app/tasks/${task.id}`) },
    onError: (error: Error) => toast.error(error.message),
  })
  if (isLoading) return <Skeleton className="h-96" />
  if (!note) return <EmptyState title="Note not found" action={<Button asChild><Link to="/app/notes">Back to notes</Link></Button>} />
  return <div className="mx-auto max-w-4xl">
    <div className="mb-4 flex items-center justify-between gap-2"><Button variant="ghost" size="sm" asChild><Link to="/app/notes"><ChevronLeft /> Notes</Link></Button><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => setPreview((value) => !value)}>{preview ? <PenLine /> : <Eye />}{preview ? 'Edit' : 'Preview'}</Button><Button size="sm" variant="secondary" onClick={() => convert.mutate()} disabled={!title.trim() || convert.isPending}><CheckSquare /> Convert to task</Button><Button size="sm" onClick={() => save.mutate()} disabled={!title.trim() || save.isPending}>Save</Button></div></div>
